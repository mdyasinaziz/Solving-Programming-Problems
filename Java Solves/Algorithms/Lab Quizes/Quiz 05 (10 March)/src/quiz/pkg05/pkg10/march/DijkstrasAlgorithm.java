...
public class DijkstrasAlgorithm {
    ...
    public static int storeSum [] = new int [9];
    ...
    public void dijkstra(int[][] adjacencyMatrix, int startVertex) {
        ...
        for (int i = 0; i < shortestDistances.length; i++) {
            sum = sum + shortestDistances [i];
        }
        storeSum [SumCount] = sum;
        SumCount++;
        
        if (SumCount == 9) {
         for (int i = 0; i < 9; i++) {
            System.out.println (storeSum [i]);
         }
         int min = storeSum [0]; 
         for (int y = 0; y < 9; y++) {
            if (min > storeSum [y]) {   
        ...
        System.out.println ("Shortest " + min + " Source " + lowest);
        }
